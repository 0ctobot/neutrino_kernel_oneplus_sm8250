...
static enum Tfa98xx_Error tfa9874_calibrate(struct tfa98xx **tfa98xx, int *speakerImpedance)
{
	...
	for (i = 0; i < tfa98xx_device_count; i++){
		...
		if (imp == 0xc00 && (buffer[2] & (2 << i))) {
			pr_info("%s: Set the default 7.399 ohm to MTP due to  spk is damage\n", __func__ );
			/* set the default 7.399 ohm to MTP */
			imp = 7399;
			/* UI show the 0 ohm */
			speakerImpedance[i] = 0;
		}
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	*/
	...
	if ((tfa98xx->tfa->dev_idx==0) || ((tfa98xx->tfa->dev_idx==1) && tfa98xx->is_dummy_codec)) {
...
static ssize_t tfa98xx_selfcheck(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < tfa98xx_device_count; i++) {
		if (g_tfa98xx[i]->i2c->addr == CHIP_RCV_ADDR)
			state_temp = &state[0];
		else
			state_temp = &state[1];

		if (g_tfa98xx[i]->dsp_fw_state != TFA98XX_DSP_FW_OK) {
			*state_temp |= CNT_NO_REGISTER;
			continue;
		}

		/*step 1, check i2s clock, if not stable, we stop and return fail*/
		tfa98xx_dsp_system_stable(g_tfa98xx[i]->tfa, &ready);
		if (!ready) {
			dev_err(&g_tfa98xx[i]->i2c->dev,
					"Device 0x%x  I2S clock not stable, please check!\n", g_tfa98xx[i]->i2c->addr);
			*state_temp |= I2SCLK_ERROR;
			continue;
		}

		/*step 2, check alarm for OTP/UVP/OVP/OCP */
		ret = regmap_read(g_tfa98xx[i]->regmap, 0x10, &reg10h);
		if (ret < 0) {
			dev_err(&g_tfa98xx[i]->i2c->dev, "Failed to read 0x%x 10h-STATUS_FLAGS0\n", g_tfa98xx[i]->i2c->addr);
			*state_temp |= REGMAP_ERROR;
			continue;
		}
		/*bit 5 is MANALARM*/
		if(reg10h & 0x20){
			dev_info(&g_tfa98xx[i]->i2c->dev, "Device 0x%x is in alarm state\n", g_tfa98xx[i]->i2c->addr);
			/*bit3 == 1 is OCP*/
			if(reg10h & 0x8){
				dev_err(&g_tfa98xx[i]->i2c->dev, "Device 0x%x is OCP!\n", g_tfa98xx[i]->i2c->addr);
				*state_temp |= OCP_ERROR;
			}
			/*bit2 == 0 is OTP*/
			if(!(reg10h & 0x4)){
				dev_err(&g_tfa98xx[i]->i2c->dev, "Device 0x%x is OTP!\n", g_tfa98xx[i]->i2c->addr);
				*state_temp |= OTP_ERROR;
			}
			/*bit4 == 0 is UVP*/
			if(!(reg10h & 0x10)){
				dev_err(&g_tfa98xx[i]->i2c->dev, "Device 0x%x is UVP!\n", g_tfa98xx[i]->i2c->addr);
				*state_temp |= UVP_ERROR;
			}
			/*(13Hbit8 == 0) is OVP*/
			ret = regmap_read(g_tfa98xx[i]->regmap, 0x13, &reg13h);
			if (ret < 0) {
				dev_err(&g_tfa98xx[i]->i2c->dev, "Failed to read 0x%x 13h-STATUS_FLAGS3\n", g_tfa98xx[i]->i2c->addr);
				*state_temp |= REGMAP_ERROR;
			}
			if(!(reg13h & 0x100)){
				dev_err(&g_tfa98xx[i]->i2c->dev, "Device 0x%x is OVP!\n", g_tfa98xx[i]->i2c->addr);
...
static struct device_attribute tfa98xx_selfcheck_attr =
	__ATTR(selfcheck, 0444, tfa98xx_selfcheck, NULL);

//add for detected PA temperature
...
// modify end
	...
	//If no PA is detected, the attribute node will not be registered
	if (!dummy) {
		...
		//add for smartPA selfcheck attr
		ret = sysfs_create_file(&i2c->dev.kobj, &tfa98xx_selfcheck_attr.attr);
		if(ret < 0)
		{
			pr_err("%s sysfs_create_file tfa98xx_selfcheck_attr err.", __func__);
		}
	}
