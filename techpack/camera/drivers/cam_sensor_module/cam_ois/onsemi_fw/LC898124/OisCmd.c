...
TRACE("1st PtoP =%08x \n", ((UINT16)0xFFFF - (StTneVal.StDwdVal.UsHigVal + StTneVal.StDwdVal.UsLowVal)) ) ;	
			// 初回のPtpで既にTarget Rangeに入っているなら終了。
...
void	SetSinWavePara( UINT8 UcTableVal ,  UINT8 UcMethodVal )
{
	...
	if( UlFreqDat == 0xFFFFFFFF )			/* Sine波中止 */
	{
		...
		SetTransDataAdr124( SinWave_OutAddr	,	0x00000000 ) ;		// 出力先アドレス
		SetTransDataAdr124( CosWave_OutAddr	,	0x00000000 );		// 出力先アドレス
		...
		RamWrite32A( SinWaveC_Regsiter	,	0x00000001 ) ;									// Sine Wave Start
		SetTransDataAdr124( SinWave_OutAddr	,	(UINT32)HALL_RAM_HXOFF1 ) ;		// 出力先アドレス
		SetTransDataAdr124( CosWave_OutAddr	,	(UINT32)HALL_RAM_HYOFF1 ) ;		// 出力先アドレス
